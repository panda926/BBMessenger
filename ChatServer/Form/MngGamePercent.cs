...
{
    public partial class MngGamePercent : Form
    {
        private static MngGamePercent _instance = null;
        public string _searchId = string.Empty;

        public bool _isSaveUserList = false;
        public List<UserInfo> _saveList = new List<UserInfo>();

        public static MngGamePercent GetInstance()
        {
            if( _instance == null )
                _instance = new MngGamePercent();

            return _instance;
        }

        public MngGamePercent()
        {
            InitializeComponent();
        }
        ...
        public void RefreshRecommenderList()
        {
            ...
            string queryStr = string.Format("select * from tblUser where Kind = {0} ", (int)UserKind.RecommendOfficer);
