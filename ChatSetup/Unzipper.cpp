...
// simple interface
bool CUnzipper::Unzip(char* outputFolder)
{
	if (!m_uzFile)
		return FALSE;

	strcpy( m_szOutputFolder, outputFolder );

	return UnzipTo(m_szOutputFolder, false);
}
...
bool CUnzipper::UnzipTo(LPCTSTR szFolder, bool bIgnoreFilePath)
{
	...
	HWND hWnd = AsyncDownloader::GetInstance()->m_pParent->m_hWnd;
	...
	do
	{
		...
		downloadStatus.ulProgress = fileIndex;
		downloadStatus.ulProgressMax = fileCount;

		::SendMessage(hWnd, WM_USER_DISPLAYSTATUS,
...
void UnterminatePath2(CString& sPath)
{
	sPath.TrimRight();
...
bool CUnzipper::CreateFolder(LPCTSTR szFolder)
{
	...
	while (!bDone && bResult)
	{
		...
		if (GetFileAttributes(sFolder) == 0xffffffff && ::CreateDirectory(sFolder, NULL) != true)
			bResult = false;
	}
